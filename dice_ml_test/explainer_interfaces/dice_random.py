...
class DiceRandom(ExplainerBase):
    ...
    def _generate_counterfactuals(self, query_instance, total_CFs, desired_range,  desired_class, permitted_range,
        ...
        cfs_df = None
        # query_instanceのレコードをsample_size分コピー (これの各特徴量をrandom_instanceの値に置き換える)
        ...
        for num_features_to_vary in range(1, len(self.features_to_vary)+1):
            ...
            for k in range(sample_size):
                # 各特徴量をrandom_instanceの値に置き換える
                candidate_cfs.at[k, selected_features[k][0]] = random_instances.at[k, selected_features[k][0]]
            # generated dataでの学習済みMLモデルの予測スコア(確率)
            scores = self.predict_fn(candidate_cfs)
            # 予測スコアがvalidityを満たすかどうかを判定
            validity = self.decide_cf_validity(scores)
            if sum(validity) > 0:
                # validityを満たすsampleを残す
                rows_to_add = candidate_cfs[validity == 1]

                if cfs_df is None:
                    # validityを満たすサンプルを格納するdataFrame
                    cfs_df = rows_to_add.copy()
                else:
                    # validityを満たすサンプルを追加 ※ appendだと遅い!
